var app={}
var karateImgs = datos => {
    app.karateImgs=datos;
}
var logoClub = datos => {
    app.logoClub=datos;
}
...
function cargarPortada(){
    var portadaHtml="";
    portadaHtml+="<img id='imagen' src='"+app.karateImgs[0].imagen+"' alt='No se encontro la imagen' class='col-12'/>";
...
function cargarNoticia(){
    var deporte=noticia => noticia.tipo=="karate";
...
function iniciar(){
    ...
    var imagenes=app.karateImgs.map(imagen=>imagen.imagen);
    function cambiar()
    {
        img.style.opacity = 0;
