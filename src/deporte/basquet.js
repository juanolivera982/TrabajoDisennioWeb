var app={}
var basquetImgs = datos => {
    app.basquetImgs=datos;
}
var logoClub = datos => {
    app.logoClub=datos;
}
...
function cargarPortada(){
    var portadaHtml="";
    portadaHtml+="<img id='imagen' src='"+app.basquetImgs[0].imagen+"' alt='No se encontro la imagen' class='col-12'/>";
...
function cargarNoticia(){
    var deporte=noticia => noticia.tipo=="basquet";
...
function iniciar(){
    ...
    var imagenes=app.basquetImgs.map(imagen=>imagen.imagen);
    function cambiar()
    {
        img.style.opacity = 0;
